...
enable={1,1,1,1,1};
msgG=false
dservers={"192.168.250.32","192.168.250.33","192.168.250.34"}
N=3
servers={}
n,m=25,50
ciclo=-1
		...
		if(enable[i]==1) then
			...
			if(not(servers[i]:connect(dservers[i],port))) then 
				...
				if z==-1 then
					...
					servers[i]:settimeout(0.2)
					if(servers[i]:receive()) then 
						z=i
						break
          end
				end
...
function love.update(dt)
	if(ciclo==0) then
		...
		msg=nil;
		flt=false;
		if(not(actual==-1)) then ciclo=1;servers[actual]:settimeout(90) end
	elseif(ciclo==1) then
		if(actual==-1) then ciclo=3;return end
		state=servers[actual]:receive();
		if(state) then ciclo=2 else ciclo=3 end
	elseif(ciclo==2) then
		servers[actual]:settimeout(1);
		if(flt) then 
	...
	elseif(ciclo==3) then
		...
		flt=true;
		ciclo=1
		if(not(actual==-1)) then servers[actual]:settimeout(15); end
	end
end
